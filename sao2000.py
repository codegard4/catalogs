...
def connectionParameters():
    #retrieves mysql connection parameters from config file
...
def createDatabase(databaseName = "SAO2000_dev"): 
    #Creates the SAO2000 database if it doesnt already exist
    try:
...
def insertIntoTable1950(databaseName = "SAO2000_dev", numRows = 5, fileName = "sao.dat"):  
    ...
    with open(f'{fileName}', 'r') as f:
        for line in tqdm(f):
            ...
            cur.execute(sql,(SaoNumber, Dup, RA1950,
                                        PMRA_1950,PMRA_1950mu,RA2m_Flag, 
                                        RA1950_precessed,RA1950_precessed_sd, 
                                        Original_Epoch, Dec1950, PMDec_1950, 
                                        PMDec_1950mu, D2m_Flag, DE2s, e_DE2, 
...
def insertIntoTable2000(databaseName = "SAO2000_dev", numRows = 5, fileName = "sao.dat"): 
    ...
    with open(f'{fileName}', 'r') as f:
        for line in tqdm(f):
            ...
            if(PhotMag == 99.9):
                PhotMag = None
            cur.execute(sql,(SaoNumber, RA, PMRA, Decl, PMDec, 
                                        RA_rad, Dec_rad, RA_deg, Dec_deg, 
                                        PhotMag, VMag, SpectralType,
                                        VMag_delta, PhotMag_delta))   
    conn.commit()
...
def parseArguments(in_args):
    #parses command line arguments when the python sao2000.py command is called
    ...
    parser.add_argument("-d", "--databaseName", dest="dName", type=str, help="Name of the database to insert the tables into", default="SAO2000_dev")
    parser.add_argument("-f", "--fileName", dest="fName", type=str, help="Path to the star catalog file", default="sao.dat")
    parser.add_argument("-r", "--rows", dest="rows", type=int, help="Number of rows to insert into the database", default=99999999)
    args = None
    try:
